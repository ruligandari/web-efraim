...
<div id="tambahModal" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity overflow-y-auto">
    <div class="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 transform scale-100 transition-transform relative z-10 border border-gray-100 dark:border-slate-700 my-8">
        ...
        <form action="<?= base_url('admin/soal/store') ?>" method="POST" class="space-y-4">
            <div>
            ...
            <div>
                ...
                <input type="number" name="level" placeholder="Masukkan Level (misal: 1)" required class="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary dark:text-white shadow-sm transition-all">
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-700 mt-6">
                ...
                <button type="submit" class="px-4 py-2 rounded-xl bg-primary hover:bg-blue-600 text-white transition-colors shadow-md shadow-blue-500/20 font-medium">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal<?= $s['id'] ?>" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity overflow-y-auto">
    <div class="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 transform scale-100 transition-transform relative z-10 border border-gray-100 dark:border-slate-700 my-8">
        ...
        <form action="<?= base_url('admin/soal/update/' . $s['id']) ?>" method="POST" class="space-y-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Soal</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Level</label>
                <input type="number" name="level" value="<?= htmlspecialchars($s['level'] ?? '') ?>" required class="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary dark:text-white shadow-sm transition-all">
            </div>
